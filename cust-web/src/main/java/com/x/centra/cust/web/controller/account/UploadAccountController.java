package com.x.centra.cust.web.controller.account;

import com.x.centra.cust.web.model.account.FileData;
import com.x.centra.cust.web.model.account.ImportLogShowVo;
import com.x.centra.cust.web.model.account.UploadAccountData;
import com.x.centra.cust.web.util.SftpUtil;
...
public class UploadAccountController {
    ...
    public ModelAndView toQueryResult(HttpServletRequest request) {
		return new ModelAndView("account/queryResult");
    }
...
//			String serviceTypeJSone = JSONObject.toJSONString(sysParams);
//			model.put("serviceTypeList", serviceTypeJSone);
//		}else{
//			model.put("serviceTypeList", "[]");
//		}
		return new ModelAndView("account/uploadAccount",model);
	}
...
//			}
			sftp = SftpUtil.connect("172.16.8.26", 22, "devweb01", "devweb01");
			String fileName = SftpUtil.upload("/data/devweb01/discount", uploadFile, sftp);
			FileData fileData = new FileData();
			fileData.setName(fileName);
			fileData.setPosition("172.16.8.26:/data/devweb01/discount");
	...
	public ResponseData<String> uploadFile(HttpServletRequest request, UploadAccountData uploadAccountData) {
		...
		if ("order".equals(dataType)) {
			...
			ResponseHeader responseHeader = new ResponseHeader(true, ResponseData.AJAX_STATUS_FAILURE, "上传失败");
...
//		String operId = Long.toString(userClient.getAccountId());// 操作员id
		...
		importRequest.setOptOperId("1");
		importRequest.setTenantId("BIU");
		...
		try {
			...
			if (importBillDetailData != null) {
				...
				if (resultResponseHeader.isSuccess()) {
					...
					ResponseHeader responseHeader = new ResponseHeader(true, ResponseData.AJAX_STATUS_SUCCESS, "上传成功");
					responseData.setResponseHeader(responseHeader);
				} else {
...
//		String operId = Long.toString(userClient.getAccountId());// 操作员id
		String tenantId = userClient.getTenantId();
		IStreamFileInputSV iStreamFileInputSV = DubboConsumerFactory.getService(IStreamFileInputSV.class);
		...
		fileParam.setOperId("1");
		fileParam.setTenantId("BIU");
		ResponseData<String> responseData = null;
		try {

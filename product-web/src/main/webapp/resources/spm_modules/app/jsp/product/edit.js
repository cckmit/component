    	...
    	//保存商品信息
      	_saveProd:function() {
			...
			if($("#prodForm").valid() && this._checkInput() && this._convertProdPic() && this._convertNoKeyAttr()  && this._checkAreaNum()){
				//获取editor中内容
				$("#detailConVal").val((encodeURIComponent(editDom.getData())));
				if (window.console) {
//					console.log("detailConVal = " + $('#detailConVal').val());
				}
				ajaxController.ajax({
					type: "post",
					processing: true,
...
//					console.log("====图片信息:" + $(this).attr('imgId') + ",");
				...
				if (attrVal==='0'){
					...
					pic = {'attrvalueDefId':$(this).attr('attrVal'),'vfsId':$(this).attr('imgId'),'picType':$(this).attr('imgType'),'serialNumber':$(this).attr('picInd')};
			...
			$("#noKeyAttrDiv .word3").each(function(i){
				...
				switch (valWay){
					...
					case '2'://多选
						...
						break;
					case '3'://单行输入
						val = $("#noKeyAttrDiv input[attrId='noKeyAttr"+attrId+"'")[0].value;
						attrValArray.push({'attrValId':'','attrVal':val,'attrVal2':''});
						break;
					case '4'://多行输入
						val = $("#noKeyAttrDiv textarea[attrId='noKeyAttr"+attrId+"'")[0].value;
						attrValArray.push({'attrValId':'','attrVal':val,'attrVal2':''});
						break;
...
//						fileUrl = _base + "/home/displayPic?id=" + filePosition + "&type=" + fileName;
...
//				console.log("将要设置属性图片:" + imgObj.attr('attrVal') + ",序号:" + imgObj.attr('picInd'));
			...
			imgObj.attr('height', '78');
			imgObj.attr('width', '78');
			//添加删除按钮
		...
		_checkAreaNum:function(){
			//商品预售
			var isSaleNationwide = $("input[name='isSaleNationwide']:checked").val();
			var areaNum = $("#dialogAreaNum").html();
			if ('N' === isSaleNationwide && areaNum === '0') {

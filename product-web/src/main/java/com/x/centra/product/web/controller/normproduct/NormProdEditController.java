package com.x.centra.product.web.controller.normproduct;
...
import com.x.centra.product.api.normproduct.param.AttrValRequest;
import com.x.centra.product.api.normproduct.param.NormProdInfoResponse;
import com.x.centra.product.api.normproduct.param.NormProdSaveRequest;
...
public class NormProdEditController {
	...
	public ResponseData<String> saveProductInfo(NormProdSaveRequest normInfo, HttpServletRequest request,
		...
		List<AttrValRequest> attrValList = new LinkedList<>();
		Gson gson = new Gson();
		// 标准品关键属性
		...
		if(StringUtil.isBlank(normInfo.getProductId())){
			...
			LOG.info("=====执行normProductSV.createProductAndStoGroup添加标准品信息结束,当前时间戳: {}, 用时: {}毫秒", createEnd, (createEnd-createStart));
			if(response != null){
				...
				if (header != null && !header.isSuccess()) {
					...
					responseData = new ResponseData<>(ResponseData.AJAX_STATUS_SUCCESS, "添加成功");
				}
			}
		}else{
			...
			if(response != null){
				...
				if (header != null && !header.isSuccess()) {
					...
					responseData = new ResponseData<>(ResponseData.AJAX_STATUS_SUCCESS, "更新成功");
				}
			}
		}
		return responseData;

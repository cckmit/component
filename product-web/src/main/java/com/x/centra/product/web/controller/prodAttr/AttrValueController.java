...
import com.x.centra.product.api.productcat.param.AttrVal;
import com.x.centra.product.api.productcat.param.AttrValInfo;
import com.x.centra.product.api.productcat.param.AttrValPageQuery;
import com.x.centra.product.api.productcat.param.AttrValParam;
import com.x.centra.product.api.productcat.param.AttrValUniqueReq;
import com.x.centra.product.web.model.prodAttr.ProdAttrValueInfo;
...
public class AttrValueController {
	...
	public String attrList(@PathVariable("id") String attrId, Model uiModel) {
		/*
		 * //设置属性ID Long attrIdLong = Long.valueOf(attrId).longValue();
	...
	@ResponseBody
	public ResponseData<PageInfoResponse<AttrValInfo>> queryAttrValueList(HttpServletRequest request,
			AttrValPageQuery pageQuery) {
		...
		try {
			...
			responseData = new ResponseData<PageInfoResponse<AttrValInfo>>(ResponseData.AJAX_STATUS_FAILURE, "获取信息异常");
			LOG.error("获取信息出错：", e);
		}
	...
	/*
	 * private void queryBuilder(HttpServletRequest request, AttrValPageQuery
	 * pageQuery) { //设置租户ID pageQuery.setTenantId(AdminUtil.getTenantId());
	 * 
	 * if (StringUtils.isNotBlank(request.getParameter("attrvalueDefId"))) {
	 * pageQuery.setAttrvalueDefId(request.getParameter("attrvalueDefId")); }j
	...
	private PageInfoResponse<AttrValInfo> queryAttrByAttrvalId(AttrValPageQuery pageQuery) {
		...
		if (result != null) {
			...
			if (attrValInfoList != null && !attrValInfoList.isEmpty()) {
	...
	public ResponseData<String> saveAttrValue(String attrValueListStr, HttpSession session) {
		...
		if (header != null && !header.isSuccess()) {
			responseData = new ResponseData<>(ResponseData.AJAX_STATUS_FAILURE,
	...
	public ResponseData<AttrVal> queryAttrValueById(@PathVariable("id") String attrvalueDefId) {
		...
		if (header != null && !header.isSuccess()) {
			...
			responseData = new ResponseData<AttrVal>(ResponseData.AJAX_STATUS_FAILURE,
					"获取信息失败 " + header.getResultMessage());
		} else {
			responseData = new ResponseData<>(ResponseData.AJAX_STATUS_SUCCESS, "OK", attrvalue);
		}
	...
	@ResponseBody
	public ResponseData<String> delAttr(@PathVariable("id") String attrvalueDefId, AttrValUniqueReq attrVal,
			HttpSession session) {
		ResponseData<String> responseData = new ResponseData<>(ResponseData.AJAX_STATUS_SUCCESS, "删除成功");
		IAttrAndValDefSV attrAndValDefSV = DubboConsumerFactory.getService(IAttrAndValDefSV.class);
		// 设置租户ID
		...
		if (header == null || !header.isSuccess()) {
			...
			responseData = new ResponseData<>(ResponseData.AJAX_STATUS_FAILURE, errMsg);
		}
		return responseData;
	}

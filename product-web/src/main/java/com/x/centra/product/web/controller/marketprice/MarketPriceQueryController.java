package com.x.centra.product.web.controller.marketprice;
...
import com.x.centra.product.api.normproduct.param.MarketPriceUpdate;
import com.x.centra.product.api.normproduct.param.NormProdInfoResponse;
import com.x.centra.product.api.normproduct.param.NormProdRequest;
...
import com.x.centra.product.web.controller.normproduct.NormProdQueryController;
...
//		ISysUserQuerySV sysUserQuerySV = DubboConsumerFactory.getService(ISysUserQuerySV.class);
		for (NormProdResponse normProdResponse : result.getResult()) {
			...
			if (StringUtils.isNotBlank(normProdResponse.getProductType())) {
				...
				SysParamSingleCond sysParamSingleCond = new SysParamSingleCond(AdminUtil.getTenantId(),
...
//	            	}
	...
	public ResponseData<String> updateMarketPrice(MarketPriceUpdate updatePrice,HttpSession session){
		...
		*/
		...
		//保存
		com.x.base.vo.BaseResponse response = normProductSV.updateMarketPrice(updatePrice);
		com.x.base.vo.ResponseHeader header = response.getResponseHeader();
